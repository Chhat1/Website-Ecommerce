...
<template>
  <div :class="mode.darkMode ? 'bg-[#030712]' : 'bg-[#fafafa]'" 
    ...
    <div :class="mode.darkMode ? 'bg-[#090d16] border-blue-950 shadow-black/40 ' : 'bg-white border-slate-200 shadow-slate-200/50'"
         class="w-full max-w-md rounded-3xl border shadow-2xl overflow-hidden transition-all duration-500">

      <div class="h-32 relative" :class="mode.darkMode ? 'bg-slate-900' : 'bg-slate-300 '">
        ...
        <div class="mt-8 space-y-4">
          <div v-for="(item, index) in [{label: 'Full Name', val: authStore.user?.name, icon: 'person'}, {label: 'Email', val: authStore.user?.email, icon: 'envelope'}]" 
            ...
            <div>
              ...
              <h2 class="text-sm font-semibold truncate max-w-50" :class="mode.darkMode ? 'text-white' : 'text-slate-900'">{{ item.val }}</h2>
            </div>
          </div>
        </div>

        <button @click="logout" 
                class="w-full cursor-pointer mt-8 py-4 rounded-xl font-black text-[11px] uppercase tracking-widest text-white transition-all active:scale-95 shadow-lg bg-red-600 hover:bg-red-700">
          Logout
        </button>
      </div>
    </div>
